import { TextInput,Button,Text, View,FlatList, TouchableOpacity,Modal} from 'react-native';
...
export default function App() {
  ...
  const addItem = (text) => {
    if(text.length > 0){
      setList(currentList=>[...currentList,{id:Math.random().toString(),title:text}])
      setText("")
    }
  }
        ...
        <Modal 
          ...
          onRequestClose={()=>null}>
            <View style={styles.modalContainer}>
              ...
              <Text style={styles.modalTitle}>Are you sure that want to delete this item?</Text>
              <Text style={styles.modalMessage}>{itemSelected.title}</Text>
              <Button color="#6B4E71" title='Ok' onPress={()=>onHandleDelete(itemSelected.id)}/>
            </View>
        </Modal>
      </View>
    </View>
